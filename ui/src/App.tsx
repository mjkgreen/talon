...
export default function KanbanBoard() {
  ...
  const fetchSettings = async () => {
    try {
      ...
      // Show wizard so the user can configure rather than silently failing
  ...
  const selectMode = async (mode: "github" | "local" | "none") => {
    ...
    try {
      ...
      console.error("Failed to save workspace mode", e);
    }
    if (mode === "none") {
      setIsConfigured(true);
      setWizardStep(0);
        ...
        <header className="mb-8 flex justify-between items-end">
          <div>
            ...
            <div className="flex items-center gap-2 mt-2">
              <div className="text-neutral-400">Autonomous Agent Tracker</div>
              {workspaceMode === "github" && selectedRepo && (
                <span className="flex items-center gap-1.5 text-xs bg-neutral-800 border border-neutral-700 text-neutral-300 px-2 py-0.5 rounded-full">
                  <GithubLogo size={11} />
                  {selectedRepo}
                </span>
              )}
              ...
              <div className="relative">
                ...
                <div className="space-y-3">
                  ...
                  >
                    ...
                    <p className="text-xs text-neutral-500 ml-7">
                      Point Talon at a folder on this machine. Uses git worktrees when available.
                    </p>
                  </button>
                  <button
              ...
              <div className="relative">
                ...
                <div className="mb-6">
                  ...
                  <p className="text-xs text-neutral-600 mt-2">
                    Git repos get an isolated worktree per run; plain dirs are copied.
                  </p>
                </div>
                                          ...
                                          {currentIteration.subtasks.map((st: any, si: number) => {
                                            ...
                                            return (
                                              <div key={si} className="flex items-start gap-2 text-xs text-neutral-400">
                                                <span
                                                  className={`mt-0.5 shrink-0 ${stResult?.success ? "text-green-400" : stResult ? "text-red-400" : "text-neutral-600"}`}
                                                >
                                    ...
                                    <div className="p-4 border-b border-neutral-800/50">
                                      ...
                                      <div className="bg-neutral-900 p-4 rounded border border-neutral-800/50 overflow-x-auto">
                                        <pre className="text-xs text-neutral-400 font-mono whitespace-pre-wrap">
                                          {currentIteration.aggregated_output || "No output yet"}
                                        </pre>
                                      </div>
                                    ...
                                    {currentReview && (
                                      <div className="p-4 border-b border-neutral-800/50">
                                        ...
                                        <div className="flex items-center gap-3 mb-3">
                                          <span
                                            className={`text-xs px-2 py-1 rounded font-medium ${currentReview.verdict === "pass" ? "bg-green-500/10 text-green-400" : "bg-amber-500/10 text-amber-400"}`}
                                          >
                                            {currentReview.verdict.toUpperCase()}
                                          </span>
                                          <span className="text-xs text-neutral-500">
                                            Score: {Math.round((currentReview.score ?? 0) * 10)}/10
                                          </span>
                                        </div>
                  ...
                  {!activeRunState && !runError && !loadingRunState && selectedIssue.run_id && (
                    <div className="p-8 text-center text-neutral-500 border border-neutral-800/50 border-dashed rounded-xl">
                      <div className="mb-2">
                        ...
                        <code className="bg-neutral-800 px-1 py-0.5 rounded text-xs">{selectedIssue.run_id}</code>
                      </div>
