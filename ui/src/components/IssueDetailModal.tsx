...
import { Activity, AlertCircle, FileText, Pause, Play, RefreshCw, RotateCcw, Settings, Video, X } from "lucide-react";
...
export function IssueDetailModal({
  ...
  const handleRegeneratePlan = async () => {
    if (
      !confirm(
        "Are you sure you want to regenerate the implementation plan? This will clear any comments you've added.",
      )
    )
      return;
    setIsActionPending(true);
  ...
  const missingAuth =
    !!activeProject && !activeProject.test_user && !activeProject.test_password && !activeProject.cookie_file;
      ...
      <div className="bg-neutral-900 border border-neutral-800 p-8 rounded-2xl w-full max-w-4xl shadow-2xl relative overflow-hidden flex flex-col h-[90vh]">
        ...
        <div className="flex items-start gap-4 mb-6 pr-12">
          ...
          <div>
            <div className="flex items-center gap-3 mb-1">
              <span className="text-xs font-mono text-neutral-500 bg-neutral-950 px-2 py-1 rounded">T-{issue.id}</span>
        ...
        <div className="flex-1 overflow-y-auto pr-2 space-y-6">
                          ...
                          return (
                            <div className="flex items-center gap-2 text-xs text-neutral-500 font-mono">
                              <span title="Total tokens">
                                {totalTok >= 1000 ? `${(totalTok / 1000).toFixed(1)}k` : totalTok} tok
                              </span>
                              {cachePct > 0 && (
                      ...
                      <div className="p-8 text-center text-neutral-500 text-sm flex flex-col items-center gap-3">
                        {activeRunState?.status === "paused" ? (
                          <>
                            ...
                            <span>
                              Agent is paused. You can resume or restart execution using the controls at the top right.
                            </span>
                          </>
                        ) : (
                          <>
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center bg-neutral-950/40 p-4 border border-neutral-800 rounded-xl">
                <div>
                  ...
                  <p className="text-xs text-neutral-500">
                    Run or retry browser automation and screenshot validation for this task.
                  </p>
                </div>
                <button
                  ...
                  title={
                    issue.status !== "Done" && issue.status !== "Failed"
                      ? "Cannot run verification until agent execution is complete"
                      : "Run browser verification / validation"
                  }
                >
                ...
                <div className="flex items-center gap-3 border border-blue-500/20 bg-blue-500/5 rounded-xl px-4 py-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-neutral-400 mt-0.5">
                      Booting dev server and preparing browser tests. Check the activity log for details.
                    </p>
                  </div>
                </div>
              )}
              ...
              {(activeRunState.browser_result || activeRunState.video_path) &&
                (() => {
                  ...
                  const isRunning =
                    !!activeRunState.verification_running ||
                    (br && (br.summary?.startsWith("Testing…") || br.summary?.startsWith("Initializing")));
                  return (
                    <>
                      {/* Summary header */}
                      {br && (
                        <div
                          className={`border rounded-xl p-4 space-y-2 ${isRunning ? "bg-blue-500/5 border-blue-500/20" : br.passed ? "bg-green-500/5 border-green-500/20" : "bg-neutral-950 border-neutral-800"}`}
                        >
                          <div className="flex items-center justify-between">
                            ...
                            <div className="flex items-center gap-3 shrink-0 ml-4">
                              {!isRunning && (
                                <span className="text-xs text-neutral-500">{Math.round(br.score * 100)}%</span>
                              )}
                              <span className="text-xs text-neutral-600">{br.steps} steps</span>
                              {isRunning ? (
                      ...
                      {/* Assertions list */}
                      {((activeRunState.browser_result?.assertions?.length || 0) > 0 ||
                        (activeRunState.browser_result?.planned_assertions?.length || 0) > 0) &&
                        (() => {
                                ...
                                {displayItems.map((item, i) => (
                                  <div key={i} className="flex items-start gap-3 p-3">
                                    ...
                                    <div className="min-w-0">
                                      <p
                                        className={`text-sm ${item.status === "pending" ? "text-neutral-500" : "text-neutral-200"}`}
                                      >
                                        {item.description}
                                      </p>
                                      {item.selector && (
          ...
          {macroTab === "trace" && !activeRunState && !runError && !loadingRunState && issue.run_id && (
            <div className="p-8 text-center text-neutral-500 border border-neutral-800/50 border-dashed rounded-xl">
              <div className="mb-2">
                No logs found for run <code className="bg-neutral-800 px-1 py-0.5 rounded text-xs">{issue.run_id}</code>
              </div>
